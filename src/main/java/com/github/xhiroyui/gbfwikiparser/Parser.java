...
public class Parser {
    ...
    public static class Builder {
        ...
        public Builder(String query, WikiObjectType type, boolean multipleResults) {
            this.type = type;
            this.query = query;
        }
            ...
            try {
                switch (this.type) {
                    case CHARACTER:
                        return null;
                    case WEAPON:
                        return new Parser(new URL(String.format("https://gbf.wiki/api.php?action=cargoquery&format=json&tables=weapons&fields=name,title,grp=group,rarity,element,type,jpname=jpName,jptitle=jpTitle,release_date=releaseDate,obtain,obtain_text=obtainText,parent,img_full=imgFull,img_icon=imgIcon,img_square=imgSquare,img_tall=imgTall,evo_min=evoMin,evo_base=evoBase,evo_max=evoMax,evo_red=evoRed,atk1,atk2,atk3,atk4,hp1,hp2,hp3,hp4,ca1_name=ougi1Name,ca1_desc=ougi1Desc,ca2_name=ougi2Name,ca2_desc=ougi2Desc,s1_icon=skill1Icon,s1_name=skill1Name,s1_lvl=skill1Level,s1_desc=skill1Desc,s1u1_icon=skill1Upgrade1Icon,s1u1_name=skill1Upgrade1Name,s1u1_lvl=skill1Upgrade1Level,s1u1_desc=skill1Upgrade1Desc,s2_icon=skill2Icon,s2_name=skill2Name,s2_lvl=skill2Level,s2_desc=skill2Desc,s2u1_icon=skill2Upgrade1Icon,s2u1_name=skill2Upgrade1Name,s2u1_lvl=skill2Upgrade1Level,s2u1_desc=skill2Upgrade1Desc,bullets,bullet1,bullet2,bullet3,bullet4,bullet5,bullet6,bullet7,bullet8,bullet9&limit=1&order_by=name+ASC&where=name%%20LIKE%%20%%22%%%s%%%%22",query)), this.type);
                    case SUMMON:
                        return null;
                    case SM_SKILL:
                        return new Parser(new URL(String.format("https://gbf.wiki/api.php?action=cargoquery&format=json&tables=weapon_sm_skills&fields=name,rarity,rarity_text=rarityText,element,element_text=elementText,type,type_text=typeText,slot,desc_text=descriptionText,cd,cd_text=cdText,dur=duration,dur_text=durationText,cost,cost_text=costText&limit=1&order_by=name+ASC&where=name%%20LIKE%%20%%22%%%s%%%%22",query)), this.type);
                    case CLASS_SKILL:
                        return new Parser(new URL(String.format("https://gbf.wiki/api.php?action=cargoquery&format=json&tables=class_skill&fields=ix,family,row,class=job,icon,name,description,cooldown,duration,emp_cost=empCost,notes,ex&limit=5&order_by=name+ASC&where=name%%20LIKE%%20%%22%%%s%%%%22",query)), this.type);
                    case EVENT:
                        return new Parser(new URL(String.format("https://gbf.wiki/api.php?action=cargoquery&format=json&tables=event_history&fields=name,grp,enname=enName,jpname=jpName,wiki_page=wikiPage,external_url=externalUrl,time_known=timeKnown,time_start=timeStart,time_end=timeEnd,utc_start=utcStart,utc_end=utcEnd,jst_start=jstStart,jst_end=jstEnd,jpwiki_url=jpwikiUrl,jpwiki_name=jpwikiName,image,enimage=enImage,enimage_soon=enImageSoon,jpimage=jpImage,jpimage_soon=jpImageSoon,unf_element=unfElement&limit=500&order_by=name+ASC&where=name%%20LIKE%%20%%22%%%s%%%%22", query)), this.type);
                    default:
                        return null;
                }
    ...
    public GBFWikiObject parse() {
        ...
        switch (this.type) {
            case CHARACTER:
                return new GBFWikiObject(parseCharacter());
            case WEAPON:
                return new GBFWikiObject(parseWeapon());
            case SUMMON:
                return new GBFWikiObject(parseSummon());
            case SM_SKILL:
                return new GBFWikiObject(parseSmSkill());
            case CLASS_SKILL:
                return new GBFWikiObject(parseClassSkill());
            case EVENT:
    ...
    public GBFCharacter parseCharacter() {
        try (InputStreamReader reader = new InputStreamReader(this.url.openStream())) {
            ...
            return new Gson().fromJson(jsonContent, GBFCharacter.class);
    ...
    public GBFSummon parseSummon() {
        try (InputStreamReader reader = new InputStreamReader(this.url.openStream())) {
            JsonParser jsonParser = new JsonParser();
            JsonObject jsonContent = jsonParser.parse(reader).getAsJsonObject().get("cargoquery").getAsJsonArray().get(0).getAsJsonObject().get("title").getAsJsonObject();
            return new Gson().fromJson(jsonContent, GBFSummon.class);
    ...
    public GBFSmSkill parseSmSkill() {
        try (InputStreamReader reader = new InputStreamReader(this.url.openStream())) {
            JsonParser jsonParser = new JsonParser();
            JsonObject jsonContent = jsonParser.parse(reader).getAsJsonObject().get("cargoquery").getAsJsonArray().get(0).getAsJsonObject().get("title").getAsJsonObject();
            return new Gson().fromJson(jsonContent, GBFSmSkill.class);
    ...
    public GBFClassSkill parseClassSkill() {
        try (InputStreamReader reader = new InputStreamReader(this.url.openStream())) {
            JsonParser jsonParser = new JsonParser();
            JsonObject jsonContent = jsonParser.parse(reader).getAsJsonObject().get("cargoquery").getAsJsonArray().get(0).getAsJsonObject().get("title").getAsJsonObject();
            return new Gson().fromJson(jsonContent, GBFClassSkill.class);
    ...
    public GBFEvent parseEvent() {
        try (InputStreamReader reader = new InputStreamReader(this.url.openStream())) {
            JsonParser jsonParser = new JsonParser();
            JsonObject jsonContent = jsonParser.parse(reader).getAsJsonObject().get("cargoquery").getAsJsonArray().get(0).getAsJsonObject().get("title").getAsJsonObject();
            return new Gson().fromJson(jsonContent, GBFEvent.class);
